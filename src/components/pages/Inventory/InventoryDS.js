    ...
    (rack.bins || []).forEach((bin) => {
      (bin.items || []).forEach((item) => {
        ...
        if (existing) {
          ...
          materialMap.set(id, {
            total_qty: item.qty,
            min_qty: item.stock_item.min_qty,
          });
        }
      });
    });
...
function filterInventoryData(inventoryData, activeFilter) {
  ...
  materialMap.forEach(({ total_qty, min_qty }, id) => {
    if (activeFilter === "critical" && total_qty <= min_qty)
      matchingIds.add(id);
    if (
      activeFilter === "low" &&
      total_qty > min_qty &&
      total_qty <= min_qty * 1.5
    )
      matchingIds.add(id);
  });
  return (inventoryData || [])
    .map((rack) => {
      const filteredBins = (rack.bins || [])
        .map((bin) => {
          const filteredItems = (bin.items || []).filter((item) =>
            matchingIds.has(item.stock_item.id),
          );
    ...
    <ControlCenter item={componentList["CONTROL_CENTER"]}>
                  ...
                  {searchTerm && !isSearching && (
                    <span
                      style={{
                        fontSize: "10px",
                        color:
                          highlightedIds.length > 0 ? "#b58ef7" : "#6c757d",
                        fontWeight: "600",
                      }}
                    >
                      {highlightedIds.length > 0
                        ? `${highlightedIds.length} matched`
                        : "No match"}
                    </span>
                  )}
        ...
        {!hasData && (
          ...
          >
            ...
            <div style={{ fontSize: "13px" }}>
              Select a warehouse from the dropdown above to view inventory.
            </div>
          </div>
        )}
      </div>

      {/* Pending Returnables Modal */}
      {showReturnablesModal && (
        <div
          ...
          onClick={() => setShowReturnablesModal(false)}
        >
          <div
            style={{
              backgroundColor: "#fff",
            ...
            <div style={{ overflowY: "auto", flex: 1, padding: "16px 20px" }}>
              {pendingReturnables.length === 0 ? (
                ...
                >
                  ...
                  >
                    No pending returnables found
                  </div>
                </div>
              ) : (
                <div className="table-wrp background-white">
                  <Grid
                    item={componentList["gridReturnables"]}
                    ...
                    <div className="row">
                      <div className="col-12">
                        <div className="master-table-wrp">
                          <GridHeader
                            typeName="GridHeader"
                            columns={componentList["gridReturnables"].columns}
                          />
                          <GridBody
                            typeName="GridBody"
                            rows={componentList["gridReturnables"].data}
                          />
                        </div>
                      </div>
                    </div>
