...
import { generateUserProfileDisplay } from './UserProfileDS';
import config from './UserProfileCS';
...
const UserProfile = () => {
    ...
    config["buttonUpdateProfile"].event.onClick = handleUserProfile;
        ...
        try {
            if (loggedInUser !== "") {
                ...
                if (getUser && getUser !== "Error" && getUser[0].User.length > 0) {
                    ...
                    let userRole = userData.role_id !== null ? userData.role.role_code : "";

                    config["inputId"].setValue(userData.id);
                    config["inputEmail"].setValue(userData.email);
                    config["inputName"].setValue(userData.name);
                    config["inputRole"].setValue(userRole);
    ...
    return generateUserProfileDisplay(config)
}
