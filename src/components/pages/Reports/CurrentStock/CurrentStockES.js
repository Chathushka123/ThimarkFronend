...
import { generateCurrentStockDisplay } from './CurrentStockDS'
import config from './CurrentStockCS'
...
const CurrentStock = () => {
    ...
    async function handleRunReport() {
        ...
        try {
            const response = await API.get('/reports/current-stock-on-hand', {
    ...
    async function handleDownloadCsv() {
        ...
        try {
            const response = await API.get('/reports/current-stock-on-hand/download', {
                params: { query_type: queryType },
                responseType: 'blob',
            })

            // Prefer filename from Content-Disposition header
            let filename = `current_stock_on_hand_q${queryType}.csv`
    ...
    return generateCurrentStockDisplay(
        config,
